...
GSkelNode::GSkelNode() : Node("GSkelNode") {
    ...
    gskel_cfg.lkf_pn = declare_parameter<float>("gskel_lkf_pn", 0.005f);
    gskel_cfg.lkf_mn = declare_parameter<float>("gskel_lkf_mn", 0.5f);
...
void GSkelNode::publish_sparse_gvert(const std::vector<SparseVertex>& vertices, const std_msgs::msg::Header& src_header) {
    ...
    for (size_t i=0; i<vertices.size(); ++i) {
        const auto& v = vertices[i];
        MsgVertex v_msg;
        v_msg.id = v.svid; // unique vertex id
        v_msg.position.x = v.position.x;
        v_msg.position.y = v.position.y;
        v_msg.position.z = v.position.z;
        v_msg.type = v.type;
...
void GSkelNode::publish_sparse_edges(const std::vector<SparseVertex>& vertices, const std_msgs::msg::Header& src_header) {
    std::unordered_map<int,int> vid2idx;
    vid2idx.reserve(vertices.size());
    for (int i=0; i<(int)vertices.size(); ++i) vid2idx[vertices[i].svid] = i;
